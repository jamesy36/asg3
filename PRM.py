...
class PRM(object):
    ...
    def highestBallot(logs):
        ...
        highest = [0, 0]
        for ack in logs:
            if int(ack[0][0]) > ballotNum or int(ack[0][0]) \
                == ballotNum and int(ack([0][1]) > siteNum):
                ballotNum = logs[0][1]
                siteNum = logs[0][1]
            ...
            if ready[0]:
                ...
                for data in range(0, len(splitData) - 1):
                    ...
                    # BACKUP self?!?!?!?!!?
                        ...
                        if stop and process != 0 \
                            and process.find('decide') != -1:
                            key = process[1]
                            value = process[2]
                        ...
                        self.leader = True
                        prepSend = 'prepare' + str(self.ballotNum[0]) \
                            + ' ' + str(self.balltNum[1]) + '*'
                        for c in outgoingTCP.keys():
                            if c != 'cli':
                                sock = outgoingTCP.get(c)
                        ...
                        if self.ballotNum[0] < ballot \
                            or self.ballotNum[0] == ballot \
                            and self.ballotNum[1] < int(siteNum):
                            self.ballotNum[0] = ballot
                            self.ballotNum[1] = siteNum
                            prepSend = 'ack' + str(self.ballotNum[0]) \
                                + ' ' + str(self.ballotNum[1])
                        ...
                        thisAck = [balNum, acceptBal, acceptVal,
                                   rcvsiteNum]
                        ...
                        if len(self.ackList) + 1 >= 2:
                            ...
                            for c in outgoingTCP.keys():
                                if c != 'cli':
                                    sock = outgoingTCP.get(c)
                                    prepSend = 'accept' \
    + str(self.ballotNum[0]) + ' ' + str(self.ballotNum[1]) + ' ' \
    + self.acceptVal[0] + ' ' + self.acceptVal[1] + '*'
                                    sock.sendall(prepSend.encode())
                        ...
                        if self.accepts[bal] >= 1 and self.leader:
                            ...
                            log.append([self.acceptVal[0],
                                    self.acceptVal[1]])
                            backupLog.append([self.acceptVal[0],
                                    self.acceptVal[1]])
                            prepSend = 'decide' \
                                + str(self.acceptVal[0]) + ' ' \
                                + str(self.acceptVal[1])
                        for c in outgoingTCP.keys():
                            if c != 'cli':
                                sock = outgoingTCP.get(c)
                                sock.sendall(prepSend.encode())
                        ...
                        for c in incomingTCP.keys():
                            ...
                            if ready[0]:
                                data = sock.recv(1024).decode()
                        return
                    elif int(self.ballotNum[0]) < int(process[1]) \
                        or int(self.ballotNum[0]) == int(process[1]) \
                        and int(self.ballotNum[1]) <= int(process[2]):
                        propList = ''
                        ...
                        if bal in accepts:
                            ...
                            accepts[bal] = 1
                            print 'accepts is 1'
                            prepSend = 'accept' + str(process[1]) + ' ' \
                                + str(process[2]) + ' ' \
                                + str(process[3]) + ' ' + propList + '*'
                            for c in outgoingTCP.keys():
                                if c != 'cli':
                                    sock = outgoingTCP.get(c)
                                    sock.sendall(prepSend.encode())
                        ...
                        for c in outgoingTCP.keys():
                            if c == 'cli' and self.propVal[0] != None:
                                time.sleep(2)
                                prepSend = 'Winner paxos value is ' \
                                    + str(process[1])
                                sock = outgoingTCP.get(c)
